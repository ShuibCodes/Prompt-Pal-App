/**
 * ChallengeCompletionPreview — a standalone COPY of the presentational part of
 * `src/features/new-ui/QuestCompletionScreen.tsx` (medal toast, wizard
 * illustration, title, CONTINUE button).
 *
 * The live screen reads from a Convex `getQuestResult` query; this copy is fed
 * by mock props so the celebration UI can be iterated in isolation. Port the
 * JSX back into the live screen once approved.
 */
...
import type { ChallengeCompletionPreviewData } from "./mockData";
...
export interface ChallengeCompletionPreviewProps {
	data: ChallengeCompletionPreviewData;
	onContinue?: () => void;
}

export function ChallengeCompletionPreview({
	data,
	onContinue,
}: ChallengeCompletionPreviewProps) {
	const { earnedXp, questTitle } = data;

	// Celebratory buzz the moment the screen lands.
	useEffect(() => {
	...
	return (
		<SafeAreaView style={styles.container} edges={["top", "bottom"]}>
			<View style={styles.content}>
				...
				{/* Prompty Celebration — joyous, 50% bigger, celebratory bob. */}
				<View style={styles.illustrationContainer}>
					<PromptyMascot mood="joyous" size={260} celebrate entranceDelay={120} />
				</View>
			...
			{/* Confetti rains over the whole celebration. */}
			<Confetti count={110} />
		</SafeAreaView>
	);
}
